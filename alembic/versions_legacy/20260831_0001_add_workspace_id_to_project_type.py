...
def upgrade() -> None:
    ...
    if "workspace_id" not in columns:
        op.execute("ALTER TABLE project_type ADD COLUMN workspace_id INTEGER REFERENCES workspace(id)")
    ...
    if "uq_project_type_workspace_name" not in unique_constraints:
        ...
        op.execute("ALTER TABLE project_type ADD CONSTRAINT uq_project_type_workspace_name UNIQUE (workspace_id, name)")


def downgrade() -> None:
    op.execute("ALTER TABLE project_type DROP CONSTRAINT IF EXISTS uq_project_type_workspace_name")
